...
def launch(command):
    ...
    if strategy == "hb":
        ...
        new_workers = [f"{h}:{p}" for h in worker_hosts for _, p in device_to_ports]
        ...
        if local_world_size_str == "0": local_world_size_str = "1"
        horovod_command = ["horovodrun", "-np", local_world_size_str]
        subprocess_command = []
        ...
        logging.error("command is{}".format(horovod_command))
        str_horovod_command = ""
        for cmd in horovod_command:
            str_horovod_command += cmd
            str_horovod_command += " "
