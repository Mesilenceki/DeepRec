...
Status EVRestorePrepareShape(BundleReader* reader,
    ...
    TensorShape* freq_filter_shape, bool& filter_flag,
    bool& restore_filter_flag) {
  Status st = reader->LookupTensorShape(tensor_key, key_shape);
  if (!st.ok()) {
...
Status EVRestoreNoPartition(EmbeddingVar<K, V>* ev, BundleReader* reader,
  ...
  if (restore_filter_flag) {
    ...
    EVRestoreFilter(tot_key_filter_num, restore_buff,
                    ev, tensor_key, tensor_version, tensor_freq)
  }
...
Status EVRestoreDynamically(EmbeddingVar<K, V>* ev,
  ...
  for (;  ; orig_partnum++) {
    ...
    string offset_filter_tensor_name = tensor_name + "-partition_filter_offset";
    ...
    for (size_t i = 0; i < loaded_parts.size(); i++) {
      ...
      while(tot_key_num > 0) {
        ...
        reader->LookupSegmentOffset(tensor_key,
            key_part_offset , read_key_num * sizeof(K),
            restore_buff.key_buffer, key_bytes_read);
        ...
        tot_key_num -= read_key_num;
        key_part_offset += key_bytes_read;
        value_part_offset += value_bytes_read;
        version_part_offset += version_bytes_read;
        freq_part_offset += freq_bytes_read;
      }
      ...
      if (restore_filter_flag) {
        ...
        EVRestoreFilter(tot_key_filter_num, restore_buff, ev,
                        tensor_key, tensor_version, tensor_freq);
      }
    }
  }

  return ev->ImportHbmCache(name_string);
}
