...
Status ElasticTrainingPass::RewriteTrainingGraph(Graph* g, bool is_test) {
  ...
  TF_RETURN_IF_ERROR(InitEVMeta(g, ev_nodes_map, primary_ev_metas_map, opt_ev_metas_map, ev_to_origin_map));
  TF_RETURN_IF_ERROR(InitNewPartitionSubGraph(g, primary_ev_metas_map, opt_ev_metas_map, ev_nodes_map, ev_to_origin_map, is_test));
  TF_RETURN_IF_ERROR(InitNewSaveSubGraph(g, primary_ev_metas_map, opt_ev_metas_map, ev_nodes_map, ev_to_origin_map));
...
Status ElasticTrainingPass::InitEVMeta(Graph* g, std::unordered_map<std::string, Node* >& ev_nodes_map,
  ...
  for (auto& it: ev_nodes_map) {
    ...
    if (post_idx == string::npos) {
      ...
      if (opt_ev_metas_map.find(opt_name) == opt_ev_metas_map.end()) {
        opt_ev_metas_map.emplace(opt_name, 1);
        std::vector<Node*> ev_vec(partition_nums_ + 20 /* hack*/, nullptr);
        ev_vec[ev_idx] = it.second;
        ev_to_origin_map.emplace(opt_name, std::move(ev_vec));
      } else {
        opt_ev_metas_map[opt_name]++;
...
Status ElasticTrainingPass::InitNewSaveSubGraph(Graph* g,
  ...
  if (save_node_vec.size() < partition_nums_) {
    for (int i = save_node_vec.size() ; i < partition_nums_; ++i) {
      ...
      Status s;
      for (auto map : {primary_ev_metas_map, opt_ev_metas_map}) {
        for (auto& it: map) {
      ...
      new_id_shards->set_name(id_shards->name() + "/Copy");
      LOG(INFO) << "STEP 3 +++++++++";
      ...
      TF_RETURN_IF_ERROR(NodeDefBuilder(node->name() + "/Copy" + "/tensor_names", "Const")
      ...
      TF_RETURN_IF_ERROR(NodeDefBuilder(node->name() + "/Copy" + "/shape_and_slices", "Const")
      ...
      TF_RETURN_IF_ERROR(NodeDefBuilder(node->name()  + "/Copy"+ "/ev_names", "Const")
      ...
      TF_RETURN_IF_ERROR(NodeDefBuilder(node->name() + "/Copy" + "/ev_resources", "Pack")
      ...
      std::vector<DataType> ev_dtypes(kv_lookup_resource_input.size(), DT_INT64);
      std::vector<DataType> n_dtypes {DT_INT64};
      //tensor_names
      NodeDef save_node_def;
      TF_RETURN_IF_ERROR(NodeDefBuilder(node->name() + "/Copy", "SaveV3")
...
Status ElasticTrainingPass::InitNewPartitionSubGraph(Graph* g,
  ...
  for (auto it : primary_ev_metas_map) {
    ...
    int ev_partition_num = it.second;
    std::vector<Node*> nodes_to_delete;
    ...
    if (ev_partition_num < partition_nums_) {
      for (int i = ev_partition_num; i < partition_nums_; ++i) {
        ...
        std::string op_name = ori_ev_name + "/" + kPart + std::to_string(i);
        ...
        for (auto* o_node: ev_node->out_nodes()) {
          if (o_node->type_string() == "InitializeKvVariableV2Op") {
            ...
            init_node->ClearAttr("_class");
            init_node->set_name(new_ev_node->name() + "/InitializeKvVariableV2Op");
            ...
            init_value_node->ClearAttr("_class");
            init_value_node->set_name(new_ev_node->name() + "/" + init_value_edge->src()->name());
            ...
            empty_key_node->ClearAttr("_class");
            empty_key_node->set_name(new_ev_node->name() + "/" + empty_key_edge->src()->name());
            empty_key_node->set_assigned_device_name(new_device_name);
            g->AddEdge(empty_key_node, empty_key_edge->src_output(), init_node, 3);

            ev_to_primary_map.emplace(ori_ev_name, std::pair<Node*, Node*>(new_ev_node, init_node));
            break;
          }
        }
        ...
        for (auto* o_node: ev_node->out_nodes()) {
          if (o_node->type_string() == "KvResourceGather") {
            ...
            axis->ClearAttr("_class");
            axis->set_name(new_ev_node->name() + "/" + axis_edge->src()->name());
    ...
    if (!is_test) {
      if (ev_partition_num < partition_nums_) {
        TF_RETURN_IF_ERROR(ScalingUpRedistributionGraph(g, ev_vec, ev_partition_num, true));
  ...
  for (auto it : opt_ev_metas_map) {
    auto ori_ev_name = it.first;
    int ev_partition_num = it.second;
    std::vector<Node*> nodes_to_delete;
    // std::vector<Node*> ev_init_nodes_vec;
    if (ev_partition_num == partition_nums_) continue; //Do nothing
    ...
    if (ev_partition_num < partition_nums_) {
      for (int i = ev_partition_num; i < partition_nums_; ++i) {
        ...
        std::string op_name = ori_ev_name.substr(0, sep_idx) + "/" + kPart + std::to_string(i) + ori_ev_name.substr(sep_idx);
        ...
        string primary_ev_name = ori_ev_name.substr(0, sep_idx);
        ...
        for (auto* o_node: ev_node->out_nodes()) {
          if (o_node->type_string() == "InitializeKvVariableV2Op") {
            auto* init_node = g->CopyNode(o_node);
            init_node->ClearAttr("_class");
            init_node->set_name(new_ev_node->name() + "/" + o_node->name());
            init_node->set_assigned_device_name(new_device_name);
            g->AddEdge(new_ev_node, 0, init_node, 0);
            ...
            auto* primary_node = ev_to_primary_map[primary_ev_name].first;
            auto* primary_init_node = ev_to_primary_map[primary_ev_name].second;
    ...
    if (!is_test) {
      if (ev_partition_num < partition_nums_) {
        TF_RETURN_IF_ERROR(ScalingUpRedistributionGraph(g, ev_vec, ev_partition_num, false));
...
Status ElasticTrainingPass::ScalingUpBackWardGraph(Graph* g,
  ...
  std::vector<Node*> dfs_start_node;
  string opt_string;
  std::vector<Node*> backward_subgraph;
  for (auto* node: ev_node->out_nodes()) {
    if (node->IsKvSparseApply()) {
      opt_string = node->name().substr(0, node->name().find("/"));
      backward_subgraph.emplace_back(node);
      for (int i = node->num_inputs()-3; i < node->num_inputs()-1; ++i) {
        const Node* tmp_check_ev_0;
        TF_RETURN_IF_ERROR(node->input_node(i, &tmp_check_ev_0));
        backward_subgraph.emplace_back(const_cast<Node*>(tmp_check_ev_0));
        dfs_start_node.emplace_back(const_cast<Node*>(tmp_check_ev_0));
      }
    }
  }
  auto enter = [&, &backward_subgraph, opt_string](Node* n) {
    string device_name = n->assigned_device_name();
    if ((n->name().find(opt_string) != string::npos) && (!n->IsKvVarHandle())) {
      backward_subgraph.emplace_back(n);
    }
  };
  ReverseDFSFrom(*g, dfs_start_node, enter, nullptr, NodeComparatorName());
  for (int i = ev_partition_num; i < partition_nums_; ++i) {
    auto* cur_ev_node = ev_node_vec[i];
    for (auto* n: backward_subgraph) {
      LOG(INFO) << "JUNQI -------> " << n->name();
      // if ()
      // Node* new_node = g->CopyNode(n);
      // new_node->set_name(n->name() + "/Copy");
      // new_node->set_assigned_device_name(cur_ev_node->assigned_device_name());
    }
  }
...
Status ElasticTrainingPass::ScalingUpRedistributionGraph(Graph* g,
                                                       std::vector<Node*>& ev_node_vec,
                                                       int ev_partition_num, bool is_primary) {
  Status s;
  DataType key_type, value_type;
    ...
    if (i < ev_partition_num) {
      ...
      for (auto* o_node: ev_node->out_nodes()) {
        if (o_node->type_string() == "InitializeKvVariableV2Op") {
          ...
          g->AddControlEdge(o_node, filter_node);
        }
      }
      LOG(INFO) << "filter op op_def : " << filter_node->def().DebugString();
  ...
  for (int i = 0; i < partition_nums_; ++i) {
    ...
    if (i < ev_partition_num) {
      ...
      string import_op_name = ev_node->name() + "/" + "ImportStorage";
      // LOG(INFO) << "import op name : " << import_op_name;
      for (int j = 0; j < filtered_node_vec.size(); ++j) {
        if (j != i) {
      ...
      TF_RETURN_IF_ERROR(NodeDefBuilder(import_op_name, "ImportStorage")
                                        .Input(ev_node->name(), 0, ev_node->output_type(0))
                                        .Input(import_keys)
                                        .Input(import_values)
      ...
      for (auto* o_node: ev_node->out_nodes()) {
        if (o_node->type_string() == "InitializeKvVariableV2Op") {
          ...
          g->AddControlEdge(o_node, import_node);
        }
      }
      LOG(INFO) << "filter op op_def : " << import_node->def().DebugString();
      auto* first_ev_node = ev_node_vec[0];
      for (auto* n: first_ev_node->out_nodes()) {
        if (n->type_string() == "ImportStorage") {
