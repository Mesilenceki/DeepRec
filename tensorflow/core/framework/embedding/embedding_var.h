...
//Used for CPU Adaptive Embedding
  ...
  if (IsUsePersistentStorage()) {
    ...
    for (int64 i = 0; i < num_of_keys; i++) {
      ...
      char* file_addr = (char*)mmap(
          nullptr,
          sizeof(FixedLengthHeader) +
              alloc_len * sizeof(V) * (emb_config_.slot_num + 1) + key_offset,
          PROT_READ, MAP_PRIVATE, fd, 0);

      NormalContiguousValuePtr<V> tmp_value_ptr(
          alloc_, alloc_len * (emb_config_.slot_num + 1));
      void* ptr = tmp_value_ptr.GetPtr();
      memcpy(ptr, file_addr + key_offset,
             sizeof(FixedLengthHeader) +
                 alloc_len * sizeof(V) * (emb_config_.slot_num + 1));
      munmap(file_addr, sizeof(FixedLengthHeader) +
                            alloc_len * sizeof(V) * (emb_config_.slot_num + 1) +
                            key_offset);
      close(fd);
      ...
      for (int j = 0; j < emb_config_.slot_num + 1; j++) {
        ...
        if (value != nullptr) {
          value_ptr->GetOrAllocate(alloc_, value_len_, value, j, alloc_len * j);
        }
      }
