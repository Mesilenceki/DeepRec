...
### User API
GroupEmbedding provides two levels of API.The one is `tf.nn.group_embedding_lookup_sparse` and the other is `tf.feature_column.group_embedding_column_scope` which is based on feature_column API.
